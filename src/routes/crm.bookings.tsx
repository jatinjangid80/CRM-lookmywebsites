...
function BookingsPage() {
  ...
  const allBookings = useMemo(() => {
    const derived = leads
      .filter((l: any) => l.bookingReference || ["Booked", "Completed", "Confirmed", "Payment Pending", "Travel Completed", "Postponed"].includes(l.status))
      .map(
        (l: any) =>
          ({
  ...
  const handleImportBookings = (data: any[]) => {
    ...
    const importedBookings = data.map((row) => {
      ...
      const sellingPrice = parseInt(
        String(row["Selling Price (₹)"] || row["Selling Price"] || row["sellingPrice"] || row["Amount"] || row["amount"] || row["Total"] || "0")
          .replace(/[^\d.-]/g, "")
      ) || 0;

      const purchasePrice = parseInt(
        String(row["Purchase Price (₹)"] || row["Purchase Price"] || row["purchasePrice"] || "0")
          .replace(/[^\d.-]/g, "")
      ) || 0;

      const profit = parseInt(
        String(row["Profit (₹)"] || row["Profit"] || row["profit"] || "0")
          .replace(/[^\d.-]/g, "")
      ) || (sellingPrice - purchasePrice);

      const margin = sellingPrice > 0 ? Number(((profit / sellingPrice) * 100).toFixed(2)) : 0;

      const rawStatus = row["Payment Status"] || row["paymentStatus"] || row["PaymentStatus"] || row["Status"] || row["status"] || "Pending";
      let status = "Pending";
      if (rawStatus) {
        ...
        else if (s.includes("paid")) status = "Paid";
      ...
      const paid = parseInt(
        String(row["Paid"] || row["paid"] || "0")
          .replace(/[^\d.-]/g, "")
      ) || (status === "Paid" ? sellingPrice : 0);

      return {
        id: `BK-${String(currentMaxId).padStart(3, "0")}`,
        bookingType: (row["Booking Type"] || row["bookingType"] || row["Service"] || "Legacy") as Booking["bookingType"],
        supplier: row["Supplier"] || "Unknown",
        bookingDate: row["Booking Date"] || row["bookingDate"] || row["BookingDate"] || new Date().toISOString().slice(0, 10),
        customer: row["Passenger Name"] || row["PassengerName"] || row["Customer"] || row["customer"] || row["Name"] || "Unknown",
        mobileNumber: row["Mobile No"] || row["MobileNo"] || row["Mobile Number"] || row["mobileNumber"] || "",
        bookedBy: row["Booked By"] || row["bookedBy"] || row["BookedBy"] || "",
        company: row["Company"] || row["company"] || "",
        reference: row["Reference"] || row["reference"] || "",
        saleInvoiceNo: row["Sale Invoice No"] || row["saleInvoiceNo"] || "",
        purchaseInvoiceNo: row["Purchase Invoice No"] || row["purchaseInvoiceNo"] || "",
        ...
        amount: sellingPrice,
        paid,
        paymentMode: (row["Payment Mode"] || row["paymentMode"] || "Cash") as Booking["paymentMode"],
              ...
              {filteredBookings.length === 0 ? (
                ...
                filteredBookings.map((b) => (
                  <tr key={b.id} className="border-t border-border hover:bg-secondary/30 cursor-pointer transition-colors" onClick={() => handleManageBooking(b)}>
                    <td className="px-4 py-3 text-sm whitespace-nowrap">{b.bookingDate}</td>
                    ...
                    <td className="px-4 py-3 font-medium whitespace-nowrap">{formatINR(b.sellingPrice || b.amount || 0)}</td>
                    <td className="px-4 py-3 font-medium whitespace-nowrap">{formatINR(b.purchasePrice || 0)}</td>
                    <td className="px-4 py-3 font-medium whitespace-nowrap">{formatINR(b.profit || 0)}</td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap">{b.bookedBy}</td>
                    ...
                    <td className="px-4 py-3 text-sm max-w-[150px] truncate" title={b.remarks}>{b.remarks}</td>
