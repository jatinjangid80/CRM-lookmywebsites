...
function TasksPage() {
  ...
  const confirmDeleteTask = () => {
    if (taskToDelete) {
      setTasks(prev => prev.filter(t => t.id !== taskToDelete && t.parent_id !== taskToDelete));
    ...
    <div className="flex h-[calc(100vh-4rem)] flex-col bg-slate-50/50 p-6 overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5 mb-6">
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-bold text-primary">{completedTasks}</p>
        </div>
        <div className="rounded-xl border bg-white p-4 shadow-sm">
                        ...
                        <div className="space-y-1.5 overflow-y-auto max-h-[80px] pr-1 scrollbar-thin">
                          {dayTasks.map(t => (
                            <div
                              ...
                              className={`text-[10px] px-1.5 py-1 rounded truncate cursor-pointer hover:opacity-80 border ${t.status === 'Completed' ? 'bg-primary/10 text-primary border-primary/20' :
