      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {kpis.map((s) => {
          const content = (
            <div
              title={s.tooltip}
              className={`group relative overflow-hidden rounded-2xl border ${s.border} bg-card p-5 shadow-card hover:shadow-premium hover:-translate-y-0.5 transition-all duration-300 h-full`}
            >
          ...
          if (s.link) {
            return (
              <Link key={s.label} to={s.link as any} className="block cursor-pointer">
                {content}
              </Link>
            );
          }
