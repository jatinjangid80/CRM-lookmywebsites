...
export const SEED_PACKAGES: Package[] = [
  {
    ...
    description:
      "Experience the best of Dubai — towering skyscrapers, thrilling desert safari, and a stunning dhow cruise.",
    active: true,
  },
  {
    id: "PKG-002",
    ...
    description:
      "Breathtaking views from Jungfraujoch, scenic train rides, and snowy peaks at Glacier 3000.",
    active: true,
  },
  {
    id: "PKG-003",
    ...
    description:
      "Sun, sand, and crystal waters across Phuket and Krabi with island hopping by ferry.",
    active: true,
  },
  {
    id: "PKG-004",
    ...
    destination: "Vietnam",
    description:
      "Cruise through Halong Bay, explore ancient Hanoi streets, and taste authentic Vietnamese cuisine.",
    active: true,
  },
  {
    id: "PKG-005",
    ...
    description:
      "Discover the dramatic Charyn Canyon, ice-skate at Medeu, and enjoy the charm of Almaty city.",
    active: true,
  },
  {
    id: "PKG-006",
    ...
    description:
      "Trek through Kathmandu's ancient temples, sail on Phewa Lake in Pokhara, and witness Everest panoramas.",
    active: true,
  },
  {
    id: "PKG-007",
    ...
    destination: "Bhutan, Himalayas",
    description:
      "Explore the serene valleys of Bhutan — Tiger's Nest monastery, Thimphu city, and the Paro festival.",
    active: true,
  },
  {
    id: "PKG-008",
    ...
    description:
      "Romantic sunsets, luxury villas, couples spa, and private beach dinners for the perfect honeymoon.",
    active: true,
  },
  {
    id: "PKG-009",
    ...
    description:
      "Stay in heritage palaces, explore majestic forts, and experience the royal culture of Rajasthan.",
    active: true,
  },
  {
    id: "PKG-010",
    ...
    description:
      "Unwind in an overwater villa with turquoise lagoons, snorkelling reefs, and all-inclusive luxury dining.",
    active: true,
  },
];
...
function PackagesPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    const inclArr = inclInput
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const priceNum = parseInt(form.price.replace(/[^\d]/g, "")) || 0;
    const formatted = form.price.startsWith("₹")
      ? form.price
    ...
    if (editingPkg) {
      setPackages((prev) =>
        prev.map((p) =>
          p.id === editingPkg.id ? { ...p, ...form, incl: inclArr, priceNum, price: formatted } : p,
        ),
      );
    } else {
  ...
  const exportToExcel = () => {
    ...
    a.href = url;
    a.download = `packages_export_${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const exportToWord = () => {
    ...
    a.download = `packages_export_${new Date().toISOString().slice(0, 10)}.doc`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  };
  ...
  const avgPrice = packages.length
    ? Math.round(packages.reduce((s, p) => s + p.priceNum, 0) / packages.length)
    : 0;
  const topTag = (() => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage holiday packages synced with LookMyHolidays.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="rounded-2xl border border-border bg-card p-5 shadow-card">
            <div className={`inline-grid h-10 w-10 place-items-center rounded-xl ${s.color}`}>
              {s.icon}
            </div>
        ...
        <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-card shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              {editingPkg
                ? `Editing: ${editingPkg.title}`
                : "Fill in details to add a new holiday package to the catalog."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSave} className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <Label htmlFor="pkg-hotels">Hotels Details</Label>
                <Input
                  id="pkg-hotels"
                  placeholder="e.g. Hilton Resort, 5 nights"
                  value={form.hotels || ""}
                  onChange={(e) => setForm({ ...form, hotels: e.target.value })}
                />
              </div>
              <div className="space-y-1">
                <Label htmlFor="pkg-flights">Flights Included</Label>
                <Input
                  id="pkg-flights"
                  placeholder="e.g. Return Economy DEL-DXB"
                  value={form.flights || ""}
                  onChange={(e) => setForm({ ...form, flights: e.target.value })}
                />
              </div>
              <div className="space-y-1">
                <Label htmlFor="pkg-activities">Activities</Label>
                <Input
                  id="pkg-activities"
                  placeholder="e.g. City tour, Safari, Cruise"
                  value={form.activities || ""}
                  onChange={(e) => setForm({ ...form, activities: e.target.value })}
                />
              </div>
              <div className="space-y-1">
                <Label htmlFor="pkg-meal">Meal Plan</Label>
                <Input
                  id="pkg-meal"
              ...
              <div className="space-y-1 col-span-2">
                <Label htmlFor="pkg-visa">Visa Options / Requirements</Label>
                <Input
                  id="pkg-visa"
                  placeholder="e.g. Visa on arrival / E-visa assisted"
                  value={form.visa || ""}
      ...
      <Dialog open={isFilesOpen} onOpenChange={setIsFilesOpen}>
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-border p-6 shadow-2xl bg-card">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground mt-1">
              Upload itineraries (PDF), brochures, photos (JPG/PNG) or pricing details for this
              package.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="rounded-xl border border-border bg-secondary/20 p-4">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {managingFilesPkg.destination} • {managingFilesPkg.nights}
                </p>
              </div>
