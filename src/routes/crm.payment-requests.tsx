...
function PaymentRequestsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const pending = visibleRequests.filter((req) => req.status === "Pending Accounts" || req.status === "Pending Super Admin").length;
    const approved = visibleRequests.filter((req) => req.status === "Approved" || req.status === "Accounts Approved").length;
    const rejected = visibleRequests.filter((req) => req.status === "Rejected").length;
    const paid = visibleRequests.filter((req) => req.status === "Paid" || req.status === "Completed").length;
          ...
          {viewingRequest && (
            <div className="space-y-6 py-4">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
                ...
                {(isAccounts || isAdmin) && viewingRequest.status === "Approved" && (
