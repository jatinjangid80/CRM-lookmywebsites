...
import type { Customer as ExtCustomer } from "@/types/supabase";
...
function AddLeadModal({
  ...
  const handleFetchCustomer = () => {
    ...
    if (!cleanPhone) {
      toast.error("Please enter a phone number to fetch.");
      return;
    }
    ...
    if (found) {
      setForm((f) => ({
        ...f,
        name: found.customer_name || f.name,
  ...
  const submit = () => {
    ...
    if (isDuplicate) {
      toast.error("A lead with this phone number or email already exists.");
      return;
    }
        ...
        <div className="flex-1 space-y-6 p-6">
          <div className="flex items-center gap-4">
            {lead.avatar ? (
              ...
              <div className={`h-16 w-16 rounded-2xl border flex items-center justify-center shrink-0 ${getAvatarColor(lead.name)}`}>
                <span className="text-3xl font-display font-bold uppercase">
                  {initials(lead.name) || "U"}
                </span>
              </div>
            )}
            <div>
          ...
          >
            <div className="flex items-center gap-2">
              {l.avatar ? (
                ...
                <div className={`h-7 w-7 rounded-lg border flex items-center justify-center shrink-0 ${getAvatarColor(l.name)}`}>
...
/* ─── Main Page ─── */
function LeadsPage() {
  const [leads, setLeads] = useLeadsManager();
  const [localEmployees] = useSupabaseTable<unknown[]>("employees", INITIAL_EMPLOYEES);
  const employees = localEmployees?.length ? localEmployees : INITIAL_EMPLOYEES;
                    ...
                    <React.Fragment key={l.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm align-top min-w-[140px]">
                          ...
                          <div className="pl-2.5 border-l-[3px] border-[#e8dfd5] py-0.5 flex flex-col gap-2.5 max-h-[52px] overflow-y-auto pr-1 custom-scrollbar">
