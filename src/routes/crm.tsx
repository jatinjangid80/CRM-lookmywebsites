...
import {
  ...
  Plane, Bell, Search, LogOut, ListChecks, Shield, Plus, User, Clock, Calendar,
  CreditCard, Briefcase, Building2
} from "lucide-react";
...
const MOCK_EMPLOYEE_DETAILS = [
  {
    id: "LMH-01", name: "suman yadav", avatar: "/avatars/suman.png",
...
function CrmLayout() {
    ...
    try {
      ...
      if (found) {
        ...
        setSelectedEmployee({
          ...
          email: auth.role === "admin" ? "insurancesolutions58@gmail.com" : "employee@lookmyholidays.in",
          phone: "+91 9887155570",
          joinDate: "2022-01-15",
          status: "Active",
          leads: 0,
  ...
  return (
    <div className="flex min-h-screen bg-secondary/30">
      {/* Sidebar */}
      <aside className={`sticky top-0 hidden h-screen ${isCompact ? 'w-20' : 'w-64'} shrink-0 flex-col border-r border-border bg-sidebar p-4 lg:flex transition-all duration-300`}>
        <Link to="/crm" className={`mb-6 flex items-center ${isCompact ? 'justify-center' : 'px-2'}`}>
          {isCompact ? (
            ...
            <img src={logoImg} alt="LookMyHolidays" className="h-12 w-auto mix-blend-multiply dark:mix-blend-normal dark:bg-white dark:p-1.5 dark:rounded-xl transition-all" />
          )}
        </Link>

        {/* Role badge */}
        {!isCompact && (
          <div className={`mb-4 flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold ${isAdmin ? "bg-primary/10 text-primary" : "bg-violet-100 text-violet-700"}`}>
        ...
        <header className="sticky top-0 z-40 flex h-16 items-center gap-4 border-b border-border bg-background/80 px-4 backdrop-blur sm:px-8">
          {isAdmin && (
            <div className="relative max-w-md flex-1">
              ...
              <Input placeholder="Search leads, bookings..." className="pl-9" />
            </div>
          )}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 border-l border-border pl-4 outline-none hover:opacity-85 transition-opacity text-left cursor-pointer">
                  <img src={auth.avatar ?? "https://i.pravatar.cc/80"} alt={auth.name} className="h-9 w-9 rounded-full object-cover ring-2 ring-primary/20" />
                  <div className="hidden sm:block">
                    <p className="text-sm font-semibold leading-tight text-foreground">{auth.name}</p>
              ...
              <DropdownMenuContent align="end" className="w-56 mt-2 rounded-xl">
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    <p className="text-xs leading-none text-muted-foreground">{auth.role === "admin" ? "Administrator" : "Team Member"}</p>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => navigate({ to: "/crm/settings" })} className="cursor-pointer gap-2 py-2">
