...
function getNavForUser(auth: AuthUser): NavItem[] {
  ...
  if (auth.role === "admin") {
    ...
    navItems = FULL_NAV.filter((n) => n.label !== "Settings");
  } else {
    // Employee role
    ...
    if (auth.name.toLowerCase().includes("nikita")) {
      navItems = FULL_NAV.filter((n) => ["Leads", "Tasks", "Settings", "Payment Requests", "Attendance", "Payments"].includes(n.label));
    } else if (auth.name.toLowerCase().includes("aman")) {
      navItems = FULL_NAV.filter((n) => ["Vendors", "Settings", "Tasks", "Payment Requests", "Attendance", "Payments"].includes(n.label));
    } else if (auth.name.toLowerCase().includes("deepak")) {
      navItems = FULL_NAV.filter((n) => [
        "Leads", "Tasks", "Visa", "Customers", "Bookings", "Documents", "Vendors", "Payment Requests", "Settings", "Attendance", "Payments"
      ].includes(n.label));
    } else if (auth.name.toLowerCase().includes("puspa")) {
      navItems = FULL_NAV.filter((n) => [
        "Leads", "Tasks", "Customers", "Quotations", "Bookings", "Documents", "Packages", "Vendors", "Payment Requests", "Settings", "Attendance", "Payments"
      ].includes(n.label));
    } else {
      navItems = FULL_NAV.filter((n) => !["Employees", "Settings", "Payments"].includes(n.label));
    }
  }

  // Rename Payment Requests based on role
  return navItems.map((n) => {
    if (n.label === "Payment Requests") {
      if (auth.role === "admin") return { ...n, label: "Final Payment Approval" };
      if (auth.role === "manager") return { ...n, label: "Payment Reviews" };
      return { ...n, label: "My Payment Requests" };
    }
    return n;
  });
}
