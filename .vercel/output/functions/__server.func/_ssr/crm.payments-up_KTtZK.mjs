...
import { C as Search, T as Plus, f as Trash2, mt as CreditCard, xt as CircleAlert, yt as CircleCheck } from "../_libs/lucide-react.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/crm.payments-up_KTtZK.js
...
function PaymentsPage() {
	...
	const allPayments = (0, import_react.useMemo)(() => {
		const derived = leads.filter((l) => l.totalAmount).map((l) => ({
	...
	const filteredPayments = allPayments.filter((p) => p.customer.toLowerCase().includes(searchTerm.toLowerCase()) || `INV-${p.id.replace("BK-", "")}`.toLowerCase().includes(searchTerm.toLowerCase()));
