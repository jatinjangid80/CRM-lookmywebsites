...
import { E as Plus, F as Map, Ft as Briefcase, J as Hotel, Mt as Calculator, Nt as Bus, O as Plane, Ot as Car, at as FileSpreadsheet, f as Trash2, h as Table2, it as FileText, n as X, p as TramFront, pt as Download, rt as File, s as Upload, st as FileImage, x as Shield } from "../_libs/lucide-react.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/crm.bookings-DSjhGAJt.js
...
function BookingsPage() {
	...
	const exportToWord = () => {
      ...
      <body><h2>Grand Journeys CRM - Bookings Export</h2><table><tr><th>ID</th><th>Customer</th><th>Package</th><th>Travel Date</th><th>Amount</th><th>Paid</th><th>Status</th></tr>${bookingList.map((b) => `<tr><td>${b.id}</td><td>${b.customer}</td><td>${b.package}</td><td>${b.travelDate}</td><td>₹${b.amount}</td><td>₹${b.paid}</td><td>${b.status}</td></tr>`).join("")}</table></body>
      </html>
    `;
	...
	const exportToPDF = () => {
		...
		const tableRows = bookingList.map((b) => `<tr><td>${b.id}</td><td>${b.customer}</td><td>${b.package}</td><td>${b.travelDate}</td><td>₹${b.amount}</td><td>₹${b.paid}</td><td>${b.status}</td></tr>`).join("");
      ...
      <body>
        ...
        <p>Generated on ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-IN")} | Total Bookings: ${bookingList.length}</p>
        <table>
	...
	const filteredBookings = bookingList.filter((b) => activeTab === "All" || b.bookingType === activeTab || activeTab === "Holiday Package" && !b.bookingType);
